...
alert 다이얼로그는 activity의 자식임
따라서 activity 생성이후에 만들어야 되므로 
recerveir에서 만들지 말고
브로드 캐스트 리시버를 activity에서 직접 객체 생성해서 쓸 수잇도록 해야 한다.

> 결과화면


## 연습문제
smsactiity에  arraylist 만들어서
받은 문자들이 리스트에 저장되서 뜨게끔(listview 형태)
